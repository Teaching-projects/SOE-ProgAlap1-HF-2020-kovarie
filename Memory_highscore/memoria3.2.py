...
class Draw(Frame):
    ...
    def __init__(self):
        ...
        Label(self, text ='Memória teszt').pack(side=TOP, padx=40)
    ...
    # Ha a flag 1, akkor kirakja a kiválasztott kártyát, majd vár egy másodpercet
        ...
        if self.flag == 1:              # Ha lenyomták a bal gombot
            ...
            if self.elag == 1:         # Első után
                if self.k[2] != self.elso[2]: # Kizárjuk, ha ugyanarra kétszer klikkel
    ...
    def kilep(self):
        print("Kilép, nem akarja elmenteni az eredményt")
        self.beo.destroy()
        self.vaszon.destroy()

    def csinal(self,name):
        if name == "":
            print("Nem adtak meg nevet!")
            self.beo.destroy()
            self.Jatek_vege()
        else:
            sor = name+'\t'+str(100-int(self.proba))+'\n'
            fileo = open('highscore.txt', 'a')
            fileo.write(sor)
            fileo.close()
            bubi.highscore()           
            self.beo.destroy()
    ...
    def Jatek_vege(self):
        ...
        self.beo = Toplevel(self)
        self.beo.title("Név megadás")
        Label(self.beo, text="Írd be a nevedet!", width = 30, font =20).pack(side=TOP) 
        ent = Entry(self.beo, bg="yellow", relief=SUNKEN, font =20 ) 
        ent.pack(side=TOP)
        btn = Button(self.beo, text="Ne mentsd el!", font =20, command=(lambda: self.kilep())).pack(side=LEFT)
        bti = Button(self.beo, text="Mentsd el!", font =20,command=(lambda: self.csinal(ent.get()))).pack(side=RIGHT)
